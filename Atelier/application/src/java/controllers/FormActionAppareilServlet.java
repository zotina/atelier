...
public class FormActionAppareilServlet extends HttpServlet {
		...
		try {
			...
			if (action != null) {
				switch (action) {
					...
					case "detailsClient":
						try {
							detailsClient(request, response, connection);
							// Envoyez une réponse ou redirigez vers une page de succès
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	private void insert(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		try {
			...
			Appareil appareil = new Appareil(id_appareil, numero_serie, desc_probleme, date_deposition, id_clientObj,
					id_modelObj);
	...
	private void detailsEmployer(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		try {
			...
			forwardRequest(request, response, "pages/detailsEmployer.jsp");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	private void detailsClient(HttpServletRequest request, HttpServletResponse response, Connection connection)
			throws Exception {
		try {
			String id_appareil = request.getParameter("id");
			Appareil appareil = Appareil.getById(id_appareil, connection);
			request.setAttribute("client", appareil.getClient());
			forwardRequest(request, response, "pages/detailsClient.jsp");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void handleUpdate(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		if (appareil != null) {
			...
			forwardRequest(request, response, "pages/formulaireAppareil.jsp");
		} else {
