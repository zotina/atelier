					...
					<tbody>
						<% if (pieceList != null) { %>
							<% for (Piece piecevalue : pieceList) { %>
								...
								<td><%= piecevalue.getTypa().getNom() %></td>
								<td><%= piecevalue.getMarque().getNom() %></td>
								<td><a href="FormActionPieceServlet?action=handleUpdate&id_piece=<%=piecevalue.getId_piece() %>"><i class="far fa-edit"></i></a><a href="FormActionPieceServlet?action=delete&id_piece=<%=piecevalue.getId_piece() %>"><i class="far fa-trash-alt"></i></a></td>
								</tr>
							<% } %>
						<% } %>
						</tbody>
